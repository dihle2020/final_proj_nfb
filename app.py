...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:<Pass>@localhost:5432/final_project'
...
@app.route('/load_domain/<domain>', methods=['GET', 'POST'])
def load_domain(domain):
    nfbs_json = {'nfbs': []}
    nfbs = NFB.query.filter(NFB.element == domain)
    for aRec in nfbs:
        rec_info = aRec.__dict__
        del rec_info['_sa_instance_state']
        del rec_info['flag']
        del rec_info['flag_description']
        del rec_info['element_code']
        del rec_info['year_code']
        nfbs_json['nfbs'].append(rec_info)
    return jsonify(nfbs_json)

@app.route('/load_items/<item>', methods=['GET', 'POST'])
def load_item(item):
    nfbs_json = {'nfbs': []}
    nfbs = NFB.query.filter(NFB.item == item)
    for aRec in nfbs:
        rec_info = aRec.__dict__
        del rec_info['_sa_instance_state']
        del rec_info['flag']
...
@app.route('/load_domain_item/<domain>/<item>', methods=['GET', 'POST'])
def load_domain_item(domain, item):
    nfbs_json = {'nfbs': []}
    nfbs = NFB.query.filter(NFB.element == domain)
