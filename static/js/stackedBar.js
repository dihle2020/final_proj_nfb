...
//in (0, maxXVal) our (0,svgW)
    ...
    init() {
        ...
        d3.select("#legend #domain").text("Domain: All");
        d3.select("#legend #select_item").text("Item: All");
    ...
    render() {
        ...
        var barChart = vis.div.selectAll('rect')
            ...
            .on('click', function (d, i) {
                ...
                var item = d3.select("#legend #select_item").text();
                item = item.substring(6);

                if( item == 'All'){
                    d3.json('/load_domain/'+vis.keys[i]).then(temp_data => {
                       ...
                       linechart = new LineChart(nest, 'Year', vis.keys[i]+" - "+ item, 'vis1');
                    });
                }else{
                    item = item.substring(0,item.indexOf(','));
                    d3.json('/load_domain_item/'+vis.keys[i]+'/'+item).then(temp_data => {
                        console.log(temp_data['nfbs']);
                        const nest = d3.nest()
                           .key(s => s.year)
                           .rollup(v => {
