    ...
    render() {
        ...
        var barChart = vis.div.selectAll('rect')
            ...
            .on('click', function (d, i) {
                ...
                var domain = d3.select("#legend #domain").text();
                domain = domain.substring(8);

                // if domain is default, search for all matches on item
                if (domain == "All"){
                    d3.json('/load_items/'+vis.keys[i]).then(temp_data => {
                       console.log(temp_data.nfbs);
                       const nest = d3.nest()
                           .key(s => s.year)
                           .rollup(v => {
                               const map = v.map(s => s.value)
                               const reducer = (accumulator, currentValue) => accumulator + currentValue;
                               const sum = map.reduce(reducer);
                               return sum;
                           })
                           .entries(temp_data.nfbs);
                       ...
                       linechart = new LineChart(nest, 'Year', domain +" - "+ vis.keys[i], 'vis1');

                    });
                }
                else{ //match on domain and value
                    domain = domain.substring(0,domain.indexOf(','));
                    d3.json('/load_domain_item/'+domain+'/'+vis.keys[i]).then(temp_data => {
                        console.log(temp_data['nfbs']);
